...
Notes
- Availability: This module is available in Hooks. It may also be used by environment and subject sources, but with strict limitations (see below). It is not available in non-HTTP contexts (e.g., pure backend workers without an HTTP response).
...
## Prohibited operations in environment and subject sources

The following operations MUST NOT be used from environment or subject sources because they send a response body. Using these from environment or subject sources can break upstream expectations and clients:
...
These APIs are only permitted from Hooks. Environment and subject sources are limited to manipulating headers and, if needed, setting an HTTP status code. See the examples below for allowed usage.
...
## Allowed usage in environment and subject sources: signaling protection mode

In an account-protection subject source you may want to hint a frontend to require a CAPTCHA
