...
- New vm pool size keys for non-worker categories: `environment_vm_pool_size`, `subject_vm_pool_size`, `hook_vm_pool_size` (fallback to backend workers if unset).
- Action VM pool size is now automatically coupled 1:1 to `action_number_of_workers` (no separate key).

### Lua subject source execution flags (New in v1.10.0)

Nauthilus now lets you control when individual Lua subject sources run, based on the authentication outcome of the current request:
...
- Local/in-memory cache (aka local cache) sets `authenticated=true` on cache hits, so subject sources configured for authenticated requests will run for cache hits as well.
