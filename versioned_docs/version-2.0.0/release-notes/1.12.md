...
### Refactoring
- Prometheus timers now include a `resource` label for more granular monitoring.

## 1.12.2

### Bug Fixes
- Excluded `/ping` and `/metrics` endpoints from brute-force throttling to ensure monitoring availability.

## 1.12.1

### Bug Fixes
- Resolved `CROSSSLOT` error in the `SlidingWindowCounter` Lua script to improve Redis Cluster compatibility.
