...
| `OAUTH2_PKCE` | no | `disabled` | PKCE mode for `authorization_code`: `disabled` or `S256` |
...
- Unknown values in `OAUTH2_PKCE` fall back to `disabled`.
...
- `OAUTH2_PKCE=S256`: sends `code_challenge_method=S256` and validates via `code_verifier` during token exchange.
