...
| cache\_flush\_script\_path   |    no    | Full path to an optional Lua cache flush script (`nauthilus_cache_flush`) used by cache flush endpoints   | /etc/nauthilus/lua-plugins.d/hooks/cache_flush.lua |
...
#### `cache_flush_script_path` callback contract

If `lua.config.cache_flush_script_path` is configured, Nauthilus executes the callback `nauthilus_cache_flush(request)` when processing:
...
1. `table|nil`: additional Redis keys to delete (array of strings)
2. `string|nil`: optional account name; when non-empty, Nauthilus skips account lookup
...
lua:
  ...
  config:
    ...
    cache_flush_script_path: ./server/lua-plugins.d/hooks/cache_flush.lua
...
function nauthilus_run_hook(request)
    ...
    if nauthilus_util.table_length(header) == 0 or header[1] ~= "application/json" then
        nauthilus_util.print_result(request.logging, result, "HTTP request header: Wrong 'Content-Type'")
        return
    end
    ...
    if not nauthilus_util.is_table(body_table) then
        nauthilus_util.print_result(request.logging, result, "HTTP request body: Result is not a table")
        return
    end
    ...
    if CATEGORIES[result.category] then
        ...
        if request.log_level == "debug" or request.log_level == "info" then
            nauthilus_util.print_result(request.logging, result)
        end
    end
