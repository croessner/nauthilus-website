...
### `idp.password_forgotten_url`
- Optional URL for the "Forgot password?" link on the IdP login page.
...
- `code_challenge_methods_supported` ([]string, currently only `["S256"]`)
...
- `name` (string)
- `client_id` (string), `client_secret` (string, optional for public clients)
...
PKCE policy for `authorization_code`:
- Only `S256` is supported (`plain` is rejected).
- Public clients must use PKCE.
- A client is considered public if `client_secret` is omitted or `token_endpoint_auth_method` is `none`.
