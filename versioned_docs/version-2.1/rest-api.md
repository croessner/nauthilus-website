...
All request-header values for these services may be URL-partially encoded and are decoded by Nauthilus before processing.
...
All incoming Nginx request-header values may be URL-partially encoded and are understood by Nauthilus.
...
:::tip Optional Lua extension
If `lua.config.cache_flush_script_path` is configured, Nauthilus executes `nauthilus_cache_flush(request)` during cache flush.
The script can return additional Redis keys to delete and an optional account name.
:::
